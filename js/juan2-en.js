document.addEventListener("deviceready", onDeviceReady, false);
    ...
    function onDeviceReady(){
        ...
        tx.executeSql('UPDATE categorias_egreso SET nombre_categoria_egreso="Indispensables/Básics" WHERE id_categoria_egreso=1');
        tx.executeSql('UPDATE categorias_egreso SET nombre_categoria_egreso="Util tools/Secundaries" WHERE id_categoria_egreso=2');
        tx.executeSql('UPDATE categorias_egreso SET nombre_categoria_egreso="Unnecessary" WHERE id_categoria_egreso=3')
        }, errorCB, successCB);
    }
    ...
    function querySuccess(tx,result){
      for (var i = 0; i < result.rows.length; i++) {
       ...
       $('#main_table').append('<tr><td class="row" style="border: 4px solid #E37474"><a href="../views/five_sub-en.html" onclick="sendId('+row['id_categoria_egreso']+')">'+row['nombre_categoria_egreso']+'</a></td></tr>');
      }
    }
